Spree.ready(function() {
  ...
  function updateSuggestedAmount() {
    ...
    $.each(checkedItems, function(i, checkbox) {
      totalPretaxRefund += parseFloat(
        $(checkbox)
          .parents("tr")
          .find(".refund-amount-input")
          .val()
      );
    });

    var displayTotal = isNaN(totalPretaxRefund)
      ? ""
  ...
  var formFields = $(
    "[data-hook='admin_customer_return_form_fields'], \
  ...
  if (formFields.length > 0) {
    ...
    formFields.find("input#select-all").on("change", function(ev) {
      var checkBoxes = $(ev.currentTarget)
        .parents("table:first")
        .find("input.add-item");
