Spree.Views.Order.CustomerDetails = Backbone.View.extend({
  initialize: function() {
    this.billAddressView = new Spree.Views.Order.Address({
      model: this.model.get("bill_address"),
      el: this.$(".js-billing-address")
    });

    this.shipAddressView = new Spree.Views.Order.Address({
      model: this.model.get("ship_address"),
      el: this.$(".js-shipping-address")
    });

    this.customerSelectView = new Spree.Views.Order.CustomerSelect({
  ...
  render: function() {
    ...
    this.$("#guest_checkout_true").prop("checked", !user_id);
